...
contract FundMe {
    ...
    function withdraw() public payable onlyOwner checkBalance {         
        ...
        // una vez que se retiran los fondos, se limpian los funders
        for(uint256 index; index < funders.length; index++){
            address funder = funders[index];
            addressToMountFunded[funder] = 0; // cada direccion asociarla con un 0
        }

        funders = new address[](0); //  reiniciar el array de addresses (con tamaño 0)
    } 
}
